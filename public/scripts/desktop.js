...
socket.on('music-play', (api) => {
        ...
        console.log('FACCIO PARTIRE: ' + api.id)
        if(currentSong && document.getElementById(currentSong)){
            document.getElementById(currentSong).querySelector("#icon-start").innerHTML = '<ion-icon name="musical-notes"></ion-icon>'
            document.getElementById(currentSong).classList.remove("active");
        }
        document.getElementById('bar').value = 0

        console.log(api.shortTitle)
        ...
        document.getElementById('song-image').style.backgroundImage = 'url('+ api.thumbnail +')'
        playButton.innerHTML = '<ion-icon name="pause" class="bigger"></ion-icon>'
        document.getElementById(api.id).querySelector("#icon-start").innerHTML = '<ion-icon name="pause"></ion-icon>'
        document.getElementById(api.id).classList.add("active");

        if ('mediaSession' in navigator) {
            console.log(currentData.media)
            ...
            navigator.mediaSession.setActionHandler('play', function() { playStop() });
            navigator.mediaSession.setActionHandler('pause', function() { playStop() });
            navigator.mediaSession.setActionHandler('stop', function() { stopSong() });
...
function playStop(){
    ...
    if (audio.paused && audio.currentTime > 0 && !audio.ended) { // se l'audio e' stoppato
...
function search(value){
    if(value){
        console.log('CERCA CON USERNAME: ' + username)
...
audio.addEventListener('timeupdate', function() {
    ...
    document.getElementById('bar').value = this.currentTime / this.duration * 100
});
